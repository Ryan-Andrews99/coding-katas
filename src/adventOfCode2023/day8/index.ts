...
export const findNextNode = (currentNode: string[], direction: string) => direction === 'L' ? currentNode[1] : currentNode[2]
...
export const traverseNodes = (
  ...
  const nextNode = findNextNode(findNode(currentNode, nodes), directions[index])
...
//part 2
export const greatestCommonDivisor = (a: number, b: number): number => a ? greatestCommonDivisor(b % a, a) : b;
export const lowestCommonMultiple = (a: number, b: number): number => a * b / greatestCommonDivisor(a, b);
const nodesEndingA = nodes.filter(node => node[0][2] === 'A')
const part2NodeMatch = (node: string): boolean => node[2] === 'Z'

start = performance.now();
console.log(nodesEndingA.map(node => traverseNodes(directions, nodes, part2NodeMatch, node[0])).reduce(lowestCommonMultiple)) //10151663816849
end = performance.now();
